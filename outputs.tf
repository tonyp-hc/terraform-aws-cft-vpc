output "public_subnets" {
  value = join(
    ...
    compact(
      split(
        ",",
        "${aws_cloudformation_stack.vpc.outputs["PublicSubnet1"]},${aws_cloudformation_stack.vpc.outputs["PublicSubnet2"]},${aws_cloudformation_stack.vpc.outputs["PublicSubnet3"]}",
      ),
    ),
  )
}
...
output "private_subnets" {
  value = join(
    ...
    compact(
      split(
        ",",
        "${aws_cloudformation_stack.vpc.outputs["PrivateSubnet1"]},${aws_cloudformation_stack.vpc.outputs["PrivateSubnet2"]},${aws_cloudformation_stack.vpc.outputs["PrivateSubnet3"]}",
      ),
    ),
  )
}
...
output "spare_subnets" {
  value = join(
    ...
    compact(
      split(
        ",",
        "${aws_cloudformation_stack.vpc.outputs["SpareSubnet1"]},${aws_cloudformation_stack.vpc.outputs["SpareSubnet2"]},${aws_cloudformation_stack.vpc.outputs["SpareSubnet3"]}",
      ),
    ),
  )
}
